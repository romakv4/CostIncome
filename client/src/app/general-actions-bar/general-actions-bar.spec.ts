...
describe('GeneralActionsBarComponent', () => {
  ...
  it('should have 3 buttons', () => {
    ...
    expect(mock.debugElement.queryAll(By.css('button')).length).toEqual(3);
    const logout = mock.debugElement.query(By.css('button[class="logOut"]')).nativeElement;
    expect(logout.innerHTML).toBe('Log out');
    const changePassword = mock.debugElement.query(By.css('button[class="changePassword"]')).nativeElement;
    expect(changePassword.innerHTML).toBe('Change password');
    const exportToCsv = mock.debugElement.query(By.css('button[class="exportToCsv"]')).nativeElement;
